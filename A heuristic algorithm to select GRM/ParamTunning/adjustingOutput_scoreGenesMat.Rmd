...
```{r}
library(devtools)
install_github("alexsanchezpla/Lheuristic")

library(Lheuristic) #from alexsanchezpla @ github
library(caret)
library(purrr)
```
...
heurGrid1 <- expand.grid(percY1 = c(0.33),
                        percY2 = c(0.66),
                        aWeightMifL1 = c(-10, -25),
                        aWeightMifLx = c( 1, 2),
                        aWeightMifL2 = c(2, 4),
                        aReqPercentsMat1 = c(10, 4),
                        aReqPercentsMat2 = c(1, 5),
                        aReqPercentsMat3 = c(5, 1),
                        aReqPercentsMat4 = c(5, 1),
                        aReqPercentsMat5 = c(10, 4)
                        )
...
#load data
expr10x <- read.csv("expression10_simulTuning.csv", row.names=1)
met10x <- read.csv("methylation10_simulTuning.csv", row.names=1)
                          ...
                          aWeightMifL2)
{
  ...
  aReqPercentsMat   <- matrix (c(aReqPercentsMat1, 20, aReqPercentsMat2, aReqPercentsMat3, 40, 20, 0, aReqPercentsMat4, aReqPercentsMat5), nrow=3, byrow=TRUE)
...
CM10x_50 <- lapply(scoresRes10x_50, function(x) confusionMatrix(data = x$logicSc1,
                                              ...
                                              positive = 'L')$table)# $byClass confusion matrix Sensitivity, specificity, etc. With $overall we obtain kappa and accuracy, with $table we obtain CM 2x2 table
...
```{r}
#load data
expr1x_50 <- read.csv("expression1_simulTuning.csv", row.names=1)
met1x_50 <- read.csv("methylation1_simulTuning.csv", row.names=1)
...
```{r}
#load data
expr10x_1000 <- read.csv("expression10_simulTuning1000.csv", row.names=1)
met10x_1000 <- read.csv("methylation10_simulTuning1000.csv", row.names=1)
