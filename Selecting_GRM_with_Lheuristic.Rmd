...
1. DA1. Expression microarrays (`DAMicroarrays.csv`) and methylation array (`DAMetilacion.csv`) on 25 cell llines `r ipacue()`
1. DA2. Expression RNAseq (`DARNAseq.csv`) and methylation array (`DAMetilacion.csv`) on 25 cell llines
1. GEO1. Expression microarrays (Illumina beadchips, `geoMicroarrays.csv`) and methylation array (Illumina 25kMethArray, `geoMetilacion.csv`) on 25 CRC samples. The data have been collected from the GEO database records: GSE25062 for the methylation data and GSE25070 for the expression data. The dataset has been created by taking the expression values available for 26 CRC tumors and matching with their corresponding methylation values.
1. `TCGA` dataset has been obtained from The Cancer Genome Atlas Database (TCGA) (Colon Adenocarcinoma (`COAD`) Nature 2012 dataset) and downloaded through the cBioportal website.
...
1. Overimpose a $3\times 3$ grid on the scatterplot. `r ipacue()`
...
* ${X}$ is the matrix of _counts_, i.e. the number of counts in each cell of the grid, `r ipacue()`
...
* `calcFreqs` counts the number of points in each cell of the grid for given vertical (defined by parameters `x1`, `x2`) and horizontal lines (defined by parameters `y1`, `y2`, `percY1`, `percY2`) `r ipacue()`
...
1. Select datasets (a pair of row-column matched matrices, one for expression and one for methylation). `r ipacue()`
2. Set parameters:
...
* `DAExprLGenesScores.pdf` `r ipacue()`
