...
shinyServer(function(input, output, clientData, session) {
  ...
  metFile <- reactive({
    ...
    metinFile <- input$metfile
     # if (is.null(metinFile)) return(invisible(NULL))
  ...
  exprFile <- reactive({
    ...
    if(!is.null(exprinFile)){
      ...
      if(dataset=="GEOAll"){
        exprFile <- read.csv2(file.path(dataDir, "GEOTrueLMetilacion.csv"), header = TRUE,
                         sep = ";", dec = ".", quote = '"', 
                         row.names = 1)
      }else{
  ...
  output$tab <- renderTable( head(metFile())) #rayos
  output$tab2 <- renderTable(head(exprFile())) #rayos
