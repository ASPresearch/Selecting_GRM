...
#' weights <- matrix (c(1,-1,-Inf,1,-1,-1,1,1,1), nrow=3, byrow=TRUE)
#' numScore (trueFreq, weights)
numScore <- function (aGrid, 
                      aWeightM=matrix (c(1,-1,-Inf,1,-1,-1,1,1,1), nrow=3, byrow=TRUE), 
                      set1.3=TRUE){
 # if (set1.3) aWeightM[1,3]=-sum(aGrid) # NO CAL. Si es vol fer es passa via la matriu de pesos
...
toReqMat <- function (numPoints, aReqPercentMat){
  return(round(aReqPercentMat*sum(numPoints)/100,0))
}
...
scoreGenesMat <- function(mets, expres, 
  ...
  for (gene in 1:Ngenes){
    ...
    numSc <- numScore (geneGrid, aWeightM=aWeightM, set1.3 = set1.3)
...
standardWeightM <- matrix (c(1,-1,-Inf,1,-1,-1,1,1,1), nrow=3, byrow=TRUE)
...
#' @param percY1, percY2 Values used to act as default for `y1`and `y2` when these are set to `NULL`
standardWeightM <- matrix (c(1,-1,-Inf,1,-1,-1,1,1,1), nrow=3, byrow=TRUE)
standardPercentsMat <- matrix (c(15, 5, 0, 10, 5, 5, 10, 10, 15), nrow=3, byrow=TRUE)
scoreGenesMatrix <- function(mets, expres,
  ...
  scores <- data.frame(logicSc=rep(FALSE, Ngenes), numericSc=rep(0,Ngenes))
  rownames(scores)<- rownames(mets)
  reqiredCounts <- toReqMat (N, reqPercentages)
  for (gene in 1:Ngenes){
    theGene <- rownames(expres)[gene]
    xVec<- mets[theGene,]
    yVec<- expres[theGene,]
    geneGrid <- calcFreqs(xMet=xVec, yExp=yVec, x1=1/3, x2=2/3,
                          y1=NULL, y2=NULL, percY1=1/3, percY2=2/3)
    binSc <-  binScore2 (geneGrid, requiredCounts)
    scores[gene, "logicSc"] <- binSc
    numSc <- numScore (geneGrid, aWeightM=aWeightM, set1.3 = set1.3)
    scores[gene, "numericSc"] <- numSc
  }
...
#'
plotGenesMat <- function(mets, expres, fileName, text4Title=NULL, plotGrid=TRUE){
    if (!is.null(fileName))
      pdf(fileName)
...
# NOTA: 
# selectGenesMat i scoresGenesMat s'assemblen massa
# Es millor suprimir scoresGenesMat ja que de la llista de l_gens podem saber-ne el nombre pero al reves no

#' selectFromGenesMat
#' \code{selectFromGenesMat} performs similarly to scoresGenesMat but instead of counting how many L-shape genes are found it returns a boolean vector that allows to select them
#' 
#' MILLOR DESCARTAR-LO i usar `el vectors que surten de `scoreGenesMat`.
#' 
selectFromGenesMat <- function(mets, expres, minN11=1, N2 = 0){
    selectedGenes <- rep(NA, nrow(expres))
    N <- dim(mets)[1]
    if (is.null(minN11)){
      N11 <- N33 <- floor(0.1*N)
    }else{
    ...
    for (gene in 1:nrow(expres)){
      theGene <- rownames(expres)[gene]
      xVec<- as.numeric(mets[theGene,])
      yVec<- as.numeric(expres[theGene,])
      geneGrid <- calcFreqs(xMet=xVec, yExp=yVec, x1=1/3, x2=2/3,
                            y1=NULL, y2=NULL, percY1=1/3, percY2=2/3)
      geneScore <- binScore (geneGrid, n11=N11, n33=N33, nMediumBand = N2)
      #    sumScores <-sumScores+as.integer(geneScore)
      selectedGenes[gene]<-geneScore
    }
    return (selectedGenes)
}


#' scoreAndPlotGenesMat
#' \code{scoreAndPlotGenesMat} is a simple wrapper that combines \code{plotGenesMat} and \code{scoreGenesMat}
#' 
#' MILLOR DESCARTAR-LO i usar `scoreGenesMat` i `plotGenesMat` encadenats.
#' 
scoreAndPlotGenesMat <- function(mets, expres, fileName, 
                                 minN11=1, N2 = 0, 
                                 plotScatterplots=TRUE,
                                 plotOnlyLshape=FALSE){
  sumScores <-0
  N <- dim(mets)[1]
  if (is.null(minN11)){
    N11 <- N33 <- floor(0.1*N)
  ...
  if (!is.null(fileName))
    pdf(file=file.path(resultsDir, fileName))
  opt<-par(mfrow=c(2,2))
  for (gene in 1:nrow(expres)){
    theGene <- rownames(expres)[gene]
    xVec<- as.numeric(mets[theGene,])
    ...
    sumScores <-sumScores+as.integer(geneScore)
    if(plotScatterplots){
      if((plotOnlyLshape & geneScore) | (!plotOnlyLshape )){
        titol <- paste (theGene, "L-shaped:", as.character(geneScore))
        plotGenSel(xMet=xVec, yExp=yVec, titleText=titol, x1=1/3, x2=2/3)
      }
    }
  }
  par(opt)
