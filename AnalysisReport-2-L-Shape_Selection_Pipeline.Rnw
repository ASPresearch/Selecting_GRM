...
The idea is to "keep-it-simple" so that the process can be followed by a non-technical reader. 
...
require(printr)
load(file.path(dadesDir,"matchedMarrRNAseqMethDataByPAIRS.Rda"))
...
#write.csv2(DAExprData, file=file.path(dadesDir,"DatosMicroarrays.csv"),row.names = TRUE)
# write.csv2(DAMetilData, file=file.path(dadesDir,"DatosMetilacion.csv"),row.names = TRUE)
...
<<cmiSelection>>=
calcAgainCMI <- FALSE
if (!file.exists("results/cmiSelections.Rda") || calcAgainCMI){
  ...
  save(selectedGenesCmiDA, selectedGenesCmiGEO, selectedGenesCmiTCGA,
    ...
    load("results/cmiSelectionsGeneNames.Rda")
}
@
...
calcAgainHeuristic <- TRUE # Com donava error l'he calculat pas a pas i he gravat els resultats
...
<<commonHeuristic3>>=
myVenn4H<- venn.diagram(x=list(DAMicroarrays=geneListLDAExpr, 
                              GEO=geneListLGEOExpr,
                              TCGA=geneListLTCGAExpr), 
                              filename=NULL, lty = "blank",  
                              fill=c("pink1", "skyblue", "mediumorchid"),
                           main="Genes in common between the three -heuristic selected- gene lists")
grid.newpage()
grid.draw(myVenn4H)
@
...
<<minMaxCoords, echo=FALSE, eval=FALSE>>=
...
<<DrawGenes2Chromosomes, eval=FALSE>>=
redraw <- TRUE
if (redraw){
